...
tags: ["No decryption oracle", "FHE-compatible"]
introduced_in:
...
implies: []
implied_by: ["CPAD", "funcCPA", "SA", "CCVA1"]
---
...
A classical result from [Bellare, Desai, Jokipii, and Rogaway (FOCS 1997)](https://web.cs.ucdavis.edu/~rogaway/papers/sym-enc.pdf) establishes that the single-challenge (Find-Then-Guess) and multiple-challenge (Left-or-Right) variants of IND-CPA are equivalent.
...
2. $\mathcal{A}$ may encrypt any message of its choice using $pk$. In the public-key setting, the adversary implicitly has access to an encryption oracle since it holds $pk$. In the private-key setting, an explicit encryption oracle is provided.
...
In the FHE setting, [Gentry (2009)](https://crypto.stanford.edu/craig/craig-thesis.pdf) established CPA as the baseline security target, a convention that has persisted throughout subsequent generations of FHE schemes.
